...
L = 0.54;            % length (m) antes estava 2
B = 0.138;           % beam (m)
m = 1.0;           % mass (kg) antes 55kg
rg = [0.197 0 -0.022]'; % CG for hull only (m) (antes estava 0.2 0 -0.2)
R44 = 0.4 * B;      % radius of gyrations (m)
R55 = 0.25 * L;
R66 = 0.25 * L;
...
B_pont  = 0.118;     % beam of one pontoon (m) (antes estava 0.25)
y_pont  = 0.232;    % distance from centerline to waterline area center (m)(antes estava 0.395)
Cw_pont = 0.7864;     % waterline area coefficient (-)(antes estava 0.75)
Cb_pont = 0.4249;      % block coefficient, computed from m = 55 kg (antes estava 0.4)
...
% (Rigid-body mass matrix and Rigid-body Coriolis and centripetal matrix due to the rotation of {b} about {n})
I3 = eye(3);
O3 = zeros(3,3);
...
w3 = sqrt( G33/M(3,3) );         
w4 = sqrt( G44/M(4,4) );
w5 = sqrt( G55/M(5,5) ); %o erro dá aqui
